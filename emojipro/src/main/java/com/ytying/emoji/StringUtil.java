...
public class StringUtil {
     ...
     */
    public static void dealExpression(Context context, SpannableString spannableString, Pattern patten, int start) throws SecurityException, NoSuchFieldException, IllegalArgumentException, IllegalAccessException {
        ...
        while (matcher.find()) {
            ...
            if(Integer.parseInt(key.substring(3, 7))>1095){
                String rid2 = "emoji_" + (Integer.parseInt(key.substring(3, 7))-1095);
                Field field2 = R.drawable.class.getDeclaredField(rid2);
                resId2 = Integer.parseInt(field2.get(null).toString());//通过上面匹配得到的字符串来生成图片资源id
            }else{
