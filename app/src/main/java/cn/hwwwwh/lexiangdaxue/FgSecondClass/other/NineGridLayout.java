...
public abstract class NineGridLayout extends ViewGroup {
    ...
    public  void setloadImageMode(int loadImageMode) {
        this.loadImageMode = loadImageMode;
    }
    ...
    public void setUrlList(List<PicBean> urlList) {
    ...
    private void refresh() {
       removeAllViews();
        ...
        if (size == 1) {
            String url = mUrlList.get(0).getPic_url();
            ...
            for (int i = 0; i < size; i++) {
                String url = mUrlList.get(i).getPic_url();
     ...
     * @param imageView
     * @param url
     * @param showNumFlag 是否在最大值的图片上显示还有未显36696699855示的图片张数
     */
    private void layoutImageView(RatioImageView imageView, int i, String url, boolean showNumFlag) {
        ...
        displayImage(imageView, url,loadImageMode);
    }
...
//        params.width = width;
    ...
    private int getListSize(List<PicBean> list) {
    ...
    protected abstract void onClickImage(int position, String url, List<PicBean> urlList);
}